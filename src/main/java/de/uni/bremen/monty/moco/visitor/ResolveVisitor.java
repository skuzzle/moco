...
public class ResolveVisitor extends VisitOnceVisitor {
	...
	public void visit(ClassDeclaration node) {
		...
		for (ResolvableIdentifier identifier : node.getSuperClassIdentifiers()) {
			TypeDeclaration type = scope.resolveProcedureType(identifier);
	...
	public void visit(VariableDeclaration node) {
		...
		node.setType(scope.resolveProcedureType(node.getTypeIdentifier()));
	}
	...
	public void visit(ParentExpression node) {
		super.visit(node);
		node.setType(node.getScope().resolveProcedureType(node.getParentIdentifier()));
	...
	public void visit(CastExpression node) {
		super.visit(node);
		node.setType(node.getScope().resolveProcedureType(node.getCastIdentifier()));
	}
	...
	public void visit(IsExpression node) {
		super.visit(node);
		node.setToType(node.getScope().resolveProcedureType(node.getIsIdentifier()));
	...
	public void visit(FunctionDeclaration node) {
		Scope scope = node.getScope();
		node.setReturnType(scope.resolveProcedureType(node.getReturnTypeIdentifier()));
		super.visit(node);
	}
	...
	public void visit(FunctionCall node) {
		...
		try {
			declaration = scope.resolveProcedureType(node.getIdentifier());
